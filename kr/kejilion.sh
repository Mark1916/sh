...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}17.  ${gl_bai}Git 버전 관리 시스템${gl_kjlan}18.  ${gl_bai}opencode AI编程助手 ${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}------------------------"
...
PYTHON_EOF
	...
	install_skill() {
		...
		while true; do
			...
			# 추천 실기 목록 출력
			echo "추천 실기 (이름을 직접 복사해서 입력하시면 됩니다):"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}45.  ${color45}도커 가속 스테이션${gl_kjlan}46.  ${color46}GitHub加速站 ${gl_huang}★${gl_bai}"
	...
	while true; do
		...
		case "$Limiting" in
			...
			13) # 修改文件权限
				...
				read -e -p "请输入权限 (如 755): " perm
				chmod "$perm" "$filename" && echo "권한이 수정되었습니다." || echo "修改失败"
				send_stats "修改文件权限"
				;;
			14) # 重命名文件
